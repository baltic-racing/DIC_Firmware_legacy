...
void dsp_break_pres(){
	...
	uint8_t bpf = 0; // fill in when datalogger is connected
	uint8_t bpr = 0; //fill in when datalogger is connected
...
void dsp_break_temp(){
	uint16_t temp_left = 0;//fill in when datalogger is connected
	uint16_t temp_right = 0; // fill in when datalogger is connected
...
void dsp_mode(){
	//get left encoder
	dsp_mde = can_data_bytes[1][0]&0b00001000;
	if (dsp_mde>1)
	dsp_mde=1;
...
void dsp_tcs(){
	...
	dsp_data[1][2][11] = 0x30 + (can_data_bytes[1][1]/2)+1;
}
...
void dsp_tps_err(){
	...
	// error CODE send by SWC (not used) 
...
{
	//Oilpressure in eigene funktion auslagern
...
void dsp_status(void){
	...
	//get new numbers //zerlegung der Integer in einzelne charackter
